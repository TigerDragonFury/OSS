...
export default function QuotationPrintPage() {
  ...
  const { data: q, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('quotations')
        .select('*, companies(name, type), quotation_items(*)')
        .eq('id', id)
        .single()
  ...
  useEffect(() => {
    if (q) document.title = `OSS Quotation - ${q.quotation_number}`
    if (searchParams?.get('auto') === '1' && q && settings !== undefined) {
      const t = setTimeout(() => window.print(), 600)
      return () => clearTimeout(t)
    }
  }, [q, settings, searchParams])
  ...
  const quoteTerms     = settings?.quotation_terms
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 28, fontWeight: 900, letterSpacing: 2 }}>QUOTATION</div>
            <div style={{ fontSize: 13, opacity: 0.85, marginTop: 4 }}>{q.quotation_number}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 mb-8">
          ...
          <div style={{ background: '#f8fafc', borderRadius: 10, padding: '16px 20px', borderLeft: '4px solid #2563eb' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: '#1e293b' }}>
              {q.client_name || q.companies?.name || '—'}
            </div>
            {q.companies && q.client_name && (
              <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>{q.companies.name}</div>
            )}
          </div>
          ...
          <div style={{ background: '#f8fafc', borderRadius: 10, padding: '16px 20px' }}>
            <div className="grid grid-cols-2 gap-y-2" style={{ fontSize: 13 }}>
              <span style={{ color: '#64748b', fontWeight: 600 }}>Quotation #</span>
              <span style={{ fontWeight: 700, color: '#1e293b' }}>{q.quotation_number}</span>
              <span style={{ color: '#64748b', fontWeight: 600 }}>Date</span>
              <span>{new Date(q.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}</span>
              <span style={{ color: '#64748b', fontWeight: 600 }}>Valid Until</span>
              <span>{q.valid_until ? new Date(q.valid_until).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' }) : '—'}</span>              {q.payment_terms && (
                <>
                  <span style={{ color: '#64748b', fontWeight: 600 }}>Payment Terms</span>
                  <span>{q.payment_terms}</span>
                </>
              )}
              {q.deposit_percent != null && Number(q.deposit_percent) > 0 && (
                <>
                  <span style={{ color: '#64748b', fontWeight: 600 }}>Deposit Required</span>
                  <span style={{ fontWeight: 700, color: '#1e3a5f' }}>{q.deposit_percent}% ({currency} {(q.total * Number(q.deposit_percent) / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })})</span>
                </>
          ...
          <div style={{ width: 280 }}>
            ...
            {q.deposit_percent != null && Number(q.deposit_percent) > 0 && (
              <div className="flex justify-between py-2" style={{ fontSize: 13, marginTop: 8, borderTop: '1px dashed #bfdbfe', paddingTop: 10 }}>
                <span style={{ color: '#2563eb', fontWeight: 600 }}>Deposit Due ({q.deposit_percent}%)</span>
                <span style={{ fontWeight: 700, color: '#2563eb' }}>{currency} {(total * Number(q.deposit_percent) / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {q.notes && (
          <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, padding: '14px 18px', marginBottom: 24, fontSize: 13 }}>
            <div style={{ fontWeight: 700, color: '#92400e', marginBottom: 6, textTransform: 'uppercase', fontSize: 11, letterSpacing: 1 }}>Notes & Terms</div>
            <div style={{ color: '#78350f', whiteSpace: 'pre-wrap', lineHeight: 1.7 }}>{q.notes}</div>
          </div>
        )}

        {/* ── Default terms ── */}
        <div style={{ background: '#f1f5f9', borderRadius: 10, padding: '14px 18px', marginBottom: 32, fontSize: 12, color: '#64748b', lineHeight: 1.8 }}>
          <div style={{ fontWeight: 700, marginBottom: 6, textTransform: 'uppercase', fontSize: 11, letterSpacing: 1 }}>Terms & Conditions</div>
          {quoteTerms ? (
            <div style={{ whiteSpace: 'pre-wrap' }}>{quoteTerms}</div>
          ) : q.payment_terms ? (
            <div style={{ whiteSpace: 'pre-wrap' }}>{q.payment_terms}</div>
          ) : (
        ...
        >
          ...
          <span>{q.quotation_number} · Generated {new Date().toLocaleDateString('en-GB')}</span>
        </div>
      </div>
      ...
      <style>{`
        @media print {
          ...
          body { margin: 0; }
          #quotation-doc { margin: 0; width: 100%; box-shadow: none; }
        }
      `}</style>
    </>
  )
