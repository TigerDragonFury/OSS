  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Vessels</h1>
    ...
    mutationFn: async (data: any) => {
      ...
      if (vessel) {
        ...
        const { data: result, error } = await supabase
          .from('vessels')
          .insert([data.vesselData])
          .select()
        if (error) {
          console.error('Insert error:', error)
          throw error
        }
        vesselId = result[0].id
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-gray-200 pt-4 mt-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Status
                </label>
